...
module.exports = {
    ...
    usage: "[song]",
    permissions: {
        channel: ["Ver_canal", "Enviar_Mensagens", "Links_Embed"],
        member: [],
    },
    aliases: ["p","tocar"],
    /**
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "Playlist_carregada") {
                    ...
                    SongAddedEmbed.setAuthor(`Playlist adicionada a lista`, message.author.displayAvatarURL());
                    SongAddedEmbed.addField("Há", `\`${Searched.tracks.length}\` músicas na lista`, false);
                    ...
                    SongAddedEmbed.setAuthor(`Adicionada a lista`, client.config.IconURL);
                    ...
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Posição na lista", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                } else {
                    return client.sendTime(message.channel, "**Nenhum resultado encontrado para - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **Não há nada sendo tocado nesse momento...**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Sem resultados para - **" + SearchString);
                else if (Searched.loadType == "PLAYLIST_LOADED") {
                    player.queue.add(Searched.tracks);
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`playlist adicionada a lista`, client.config.IconURL);
                    SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
                    SongAddedEmbed.setDescription(`[${Searched.playlist.name}](${SearchString})`);
                    SongAddedEmbed.addField("Há", `\`${Searched.tracks.length}\` músicas para tocar`, false);
                    SongAddedEmbed.addField("Duração da Playlist", `\`${prettyMilliseconds(Searched.playlist.duration, { colonNotation: true })}\``, false);
                    Searching.edit(SongAddedEmbed);
                } else {
                    player.queue.add(Searched.tracks[0]);
                    if (!player.playing && !player.paused && !player.queue.size) player.play();
                    SongAddedEmbed.setAuthor(`Adicionado a lista`, client.config.IconURL);
            ...
            console.log(e);
            return client.sendTime(message.channel, "**Sem resultados para - **" + SearchString);
        }
    },
        ...
        options: [
            {
                ...
                type: 3,
                required: true,
                description: "Toque músicas em um canal de voz",
            },
        ],
        /**
         *
