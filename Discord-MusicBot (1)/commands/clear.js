  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player)
      return client.sendTime(
        message.channel,
        "❌ | **Não há nada sendo tocado agora...**"
      );

    if (!player.queue || !player.queue.length || player.queue.length === 0)
      return client.sendTime(message.channel, "❌ | **Não há nada para limpar no momento...**");
      if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **Você deve estar em um canal de voz para limpar a lista**");
    ...
    player.queue.clear();
    await client.sendTime(message.channel, "✅ | **Lista limpa!**");
  },
